...
- Supabase Account & Qdrant Instance

### Local Setup
1. **Clone the repo**:
   ```bash
   git clone https://github.com/Trivo121/hackeval.git
   cd hackeval/backend
   ```
2. **Configure Environment**:
   Create a `.env` file based on `.env.example` with your Supabase and Qdrant credentials.
3. **Spin up services**:
   ```bash
...
- `submissions`: Central record for every team entry.
- `submission_slides`: Granular data per page, including OCR and complexity metrics.
- `problem_statements`: Track definitions for auto-matching.
- `scoring_criteria`: Custom rubrics used by the AI Judge.
- `evaluation_scores`: Final AI-generated feedback and results.
